...
float propina(float subtotal)
{
    ...
    case 3:
        ...
        {
            cout << endl
                 << "No se agrego propina a la cuenta total\n";
            total = subtotal;
        }
        else
        {
            ...
            if (confirmar == 's' || confirmar == 'S') // Se actualiza el stock aca
            {
                ...
                case 2:
                {
                    ...
                    case 1:
                    {
                        ...
                        if (saldoTarjeta.is_null())
                        {
                            cout << "Error en la verificacion de tarjeta\n";
                            pausa();
                            break;
                        }

                        long long numeroTarjeta = saldoTarjeta["tarjeta"].get<long long>();

                        float saldoActual = saldoTarjeta["saldo"].get<float>();
                        if (saldoActual >= total)
                        {
                            ...
                            ifstream archivoEntrada("banco.json");
                            json tarjetasCompleto = json::parse(archivoEntrada);

                            for (auto &tarj : tarjetasCompleto["cuentas"])
                            {
                                if (tarj["tarjeta"].get<long long>() == numeroTarjeta)
                                {
                                    tarj["saldo"] = saldoActual;
                                    break;
                                }
                            }

                            ofstream archivoSalida("banco.json");
                            archivoSalida << setw(4) << tarjetasCompleto << endl;
                        ...
                        else
                        {
                            ...
                            cout << "Tu saldo es de $" << saldoActual << endl;
